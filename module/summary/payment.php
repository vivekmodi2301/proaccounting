...
<?php
	$i=1;
	$crs=mysqli_query($con," select id,eid,ammount,paymentid,datee,payby,bankname,cheque from payment where coid=$id and  datee BETWEEN '$fdatee' AND '$tdatee' order by datee ");
	//echo "select cash.id,eid,ammount,paymentid,datee,payby,bankname,cheque from payment where coid=$id and  datee BETWEEN '$fdatee' AND '$tdatee' order by id ";
...
<tr>
	<td><?php echo $i++;?></td>
	<td><?php echo $cdata['paymentid'];?></td>
	<td><?php if($cdata['bankname']){ echo $cdata['bankname'];} else{ echo "-";}?></td>
	<td><?php if($cdata['cheque']){ echo $cdata['cheque']; } else{ echo "-";}?> </td>
	<td><?php $cname=mysqli_fetch_assoc(mysqli_query($con,"select name from edition where id=$cdata[eid]"));
	echo $cname['name']; ?></td>
